...
%--- Plot time series
ax1 = subplot('position',[0.1 0.75 0.65 0.2])
plot(t,y)
set(gca,'XLim',xlim(:))
xlabel('t')
ylabel('y')
...
xlabel('km')
ylabel('km')
title(['unsmoothed roughness normalized in the ' num2str(pmin/1e3) '-' num2str(pmax/1e3) ' km band'])
...
colormap jet
hold on
contour(t,log2(period),sig95,[-99,1],'k','linewidth',1.5);
xlabel('alongshore distance (m)')
ylabel('Period (units of t)')
set(gca,'XLim',xlim(:))
set(gca,'YLim',log2([min(period),max(period)]), ...
