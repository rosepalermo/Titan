function [dam_matrix,wave_weight_matrix,fetch_matrix,indshoreline_ordered,cells2trash,p] = get_dam_wave_pts_vislib(lake,p,X,Y)
...
% [WaveArea_cell,FetchArea_cell,~,~,~,~,~,~] = GetFetchArea_pts(indshoreline_ocw,F_lake,p.nrays,p.delta,p.dx);    
clearvars erodedind
    %         end
