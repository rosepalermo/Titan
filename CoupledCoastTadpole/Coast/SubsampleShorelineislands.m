...
while ~observer_is_valid
    ...
    for i=1:length(slccw)
        ...
        if i == k
            env{i} = ([xsub([l_ind:nsub, 1:l_ind-1]),ysub([l_ind:nsub, 1:l_ind-1])]);
        else
            env{i} = ([xsub,ysub]);
        end
%         if i>1 && polyarea(env{i}(:,1),env{i}(:,2)) == 0 % if we're on a 0 width island
%             if i == k
%             [env{i},Pobs,nbi] = island_area(env{i},Pobs,l_ind,eps);
%             else
%             [env{i},~] = island_area(env{i},[],l_ind,eps);
%             end
%         end
    end
    ...
    if in_environment(Pobs, env, epsilon)
        ...
        observer_is_valid = 1;
        if length(env)>1; % flip the islands
            for i = 2:length(env)
                env{i} = flipud(env{i});
            end
        end
