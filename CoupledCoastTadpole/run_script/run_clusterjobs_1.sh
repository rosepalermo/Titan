#!/bin/bash  
# run all my code
# sbatch rand_uniform_runs1.slurm
# sbatch rand_wave_runs1.slurm
# sbatch rand_wave_runs2.slurm
# sbatch rand_wave_runs3.slurm
# sbatch river_uniform_runs1.slurm
sbatch river_wave_runs1.slurm
sbatch river_wave_runs2.slurm
sbatch river_wave_runs3.slurm
# sbatch rand_wave_uniform_run1.slurm
# sbatch rand_wave_uniform_run2.slurm
# sbatch rand_wave_uniform_run3.slurm
