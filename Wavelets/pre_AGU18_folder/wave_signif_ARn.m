%WAVE_SIGNIF_ARn  Significance testing for the 1D Wavelet transform WAVELET
%
%  Modified to use background spectrum of an AR(n) process rather than just
%  an AR(1). - T. Perron, March 2009
%
%   SIGNIF = ...
%      wave_signif(Y,DT,SCALE,SIGTEST,fft_theor,SIGLVL,DOF,MOTHER,PARAM)
%
% INPUTS:
%
...
%             then DOF=[2,8].
%
%    fft_theor = the theoretical background spectrum for an AR(n) process
%                with the same autoregression and variance characteristics
%                as the data in y (see modified version of dowave.m). As a
%                function of increasing PERIOD (not frequency!).
%
...
%----------------------------------------------------------------------------
function signif = ...
	wave_signif_ARn(Y,dt,scale1,sigtest,fft_theor,siglvl,dof,mother,param)
...
% The following is the theoretical spectrum for an AR(1) process. Instead
% we have included fft_theor as an input argument
