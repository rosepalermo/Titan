function dowave_duplicate(y,dt,ord,xx,yy,savename)
...
orig_y = y;
% y = [flipud(y);y;flipud(y);y]; % \/\/ -- this is trash. don't do this.

% if de-trended
y = [y;y;y;y;y];

% % if not de-trended
% % if positive, add on either side, if negative, add opposite sides
% slopesign = sign(y(end)-y(1));
% if slopesign > 0
%     y = [y-y(end);y;y+y(end)]; % extend on both sides in same direction
% elseif slopesign < 0
%     y = [y+y(1);y;y-y(1)];
% end
...
plot(t,y)
set(gca,'XLim',[2*length(orig_y)+1,length(t)-2*length(orig_y)])
xlabel('t')
ylabel('y')
title('data')
hold off
...
% yy=yy(1:end-2);
figure
scatter3(xx,yy,m(2*length(orig_y)+1:length(t)-2*length(orig_y)),[],m(2*length(orig_y)+1:length(t)-2*length(orig_y)),'.')
colormap(parula)
colorbar
set(gca,'Clim',[mean(m(2*length(orig_y)+1:length(t)-2*length(orig_y)))-2*std(m(2*length(orig_y)+1:length(t)-2*length(orig_y))),mean(m(2*length(orig_y)+1:length(t)-2*length(orig_y)))+2*std(m(2*length(orig_y)+1:length(t)-2*length(orig_y)))])
view(2)
title('m')
axis equal
axis equal
...
figure()
plot(t(2*length(orig_y)+1:length(t)-2*length(orig_y)),m(2*length(orig_y)+1:length(t)-2*length(orig_y)))
title('m')
% 
% figure
% scatter3(xx,yy,v,[],v,'.')
