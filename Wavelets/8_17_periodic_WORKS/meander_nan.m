function [azi curvi deltad] = meander(x,y)
% meander.m
...
% Find whether the data series has a positive or negative slope
slopesign = sign(az(end)-az(1));

%replace last az with first az (+/-2pi)
% az(end)=az(1)+slopesign*2*pi;

%bgslope = slopesign * az(end-1)-az(1) / (d(end) - d(1));
%bgslope = slopesign * az(end-1)-az(1) / (d(end-1)-d(1));
...
%remove extra point after detrending
az = az(1:end-1);
d = d(1:end-1);
dd = dd(1:end-1);
...
di=di(:);
azi = interp1(d,az,di); % THIS HAS A NAN AT THE END! WHY?
