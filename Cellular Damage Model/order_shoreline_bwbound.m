...
[~,total_lakes,total_islands] = find_first_order_lakes(double(~lake));
land = double(~lake);
[B,L,N,A] = bwboundaries(land,8);

[~,total_lakes,total_islands,L] = find_first_order_lakes(lake);
...
% size(B,1) because it includes the boundary of the domain as a boundary
coasts = cell(size(B,1)-1); % this will hold the ordered (CW) coastlines of the lakes/seas

for ll = 1:length(lakes)
    l = lakes(ll);
    bdy = B{l};
    r1 = bdy(1,1);
    c1 = bdy(1,2);
