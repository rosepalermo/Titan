...
for k = 1:length(slccw)
    for l = 1:length(slccw{k})
        ...
        [Pobs,env,xsub,ysub,nbi] = SubsampleShoreline(slccw,k,l,binfine,bincoarse,dist,nhood,eps,epsilon);
        V{k,l} = visibility_polygon(Pobs, env, epsilon, snap_distance);
        
        % fetch & wave area & distance & cos(theta-phi)
        FetchArea{k}(l,1) = polyarea(V{k,l}(:,1),V{k,l}(:,2));
        Fetch_dist{k,l} = sqrt(sum(([x_all(l),y_all(l)] - V{k,l}).^2,2));
        % Wave weighting = (F)*cosang
        weighted_fetch_dist{k,l} = ([x_all(l),y_all(l)] - V{k,l})*nbi'./eps;
        % cos(theta - phi) = dot product of slvec and losvec
        cosang{k,l} = weighted_fetch_dist{k,l}./Fetch_dist{k,l};
        cosang{k,l}(isnan(cosang{k,l})) = 0;
        Wavepts{k,l} = [x_all(l),y_all(l)]+(V{k,l}-[x_all(l),y_all(l)]).*cosang{k,l};
        WaveArea{k,1}(l,1) = polyarea(Wavepts{l}(:,1),Wavepts{l}(:,2));
