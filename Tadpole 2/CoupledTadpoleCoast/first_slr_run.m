...
p.Kf = 5e-6;             f   %     p.Kf             Coefficient in stream power incision law (kg m^(1+2m) yr^-2)
...
%initialize random seed so it is the same IC each time
rand('seed',16)
% get IC
[init] = get_IC(p);
...
%% coast & river parameters for erosion
